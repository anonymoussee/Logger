...
def infer_llm(instruction, query, model='codellama/CodeLlama-34b-Instruct-hf',temperature=0.0):
...
def get_instruction():
    ...
    instruction = \
    """
    You have been given a Java method code which has been deleted one logging statement. Your task is to insert one and only one logging statement starts with "log.". Make sure not to modify any other code or import any package, don't reply me any other code. Pay attention to the empty block.
...
```
   ...
   * Logging statement:  LOG.trace(entry.getKey() + entry.getValue());
    ...
    The following is the java code which has been deleted a logging statement, please reply me strictly following the reply format:
    <Java code>:
    """
...
def main():
    ...
    output_folder = "../ExperimentResults/codellama"
